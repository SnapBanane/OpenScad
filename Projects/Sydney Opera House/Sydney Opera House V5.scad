...
module bottom(x, depth) {
    ...
    difference() {
        ...
        #translate([0.5,-0.01,-1.75]) scale([depth,depth,depth]) {
            difference() {
                translate([x+16.75,0,-3.5]) rotate([0,0,0]) {
                    color("#B8A693") difference() {
                        cylinder(r=20, h=2, $fn=100);
...
module ground() {
     ...
     translate([0,-4.45,-5.5]) rotate([0,0,0]) //ground plate 
        color("#B8A693") cube([15,17.75,2]);
     translate([-6,11.28,-6]) rotate([0,0,15]) //cylinder addon
        color("#6D675A") cube([6,2.5,2], center=true);
     translate([-3,12.5,-6]) rotate([0,0,0]) //ground plate addon
        color("#6D675A") cube([6,4,2], center=true);
     translate([2.7,12.125,-6]) rotate([0,0,-7.5]) //ground plate addon
        color("#6D675A") cube([6,4,2], center=true);
}
module hall1() {
    difference() {
